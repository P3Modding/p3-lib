...
fn load(pid: u32) {
    unsafe {
        ...
        let path = s!(
            r"C:\Users\Benni\repositories\p3-lib\target\i686-pc-windows-msvc\release\p3_agent.dll"
        );
        let buf_ptr = VirtualAllocEx(
            handle,
            None,
            path.as_bytes().len(),
            MEM_COMMIT,
        ...
        let thread = CreateRemoteThread(
            ...
            None,
            0,
            Some(std::mem::transmute(load_library_a_address)),
            Some(buf_ptr),
            0,
            None,
        )
